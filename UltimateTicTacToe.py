...
def minimax_score(board, square_to_play, player, depth, maxing_player, alpha=-9999, beta=+9999):
    ...
    if depth == 0 or get_board_winner(board) is not None:
        ...
        if result == player:
            if maxing_player:
                ...
                return -200
        elif result == get_opponent(player):
            if maxing_player:
                return -200
            else:
                return 200
    ...
    if maxing_player:
        ...
        for move, sq_2_play in valid_moves:
            ...
            score = minimax_score(board=new_board, square_to_play=move, player=opponent, depth=depth - 1, maxing_player=not maxing_player, alpha=alpha, beta=beta)
    ...
    # TODO tenta entender como o algoritmo funciona com mais de dois ramos de cada vez
...
def minimax_ai(board, square_to_play, player):
    ...
    # if able to play in any board, get human input to reduce the time
    #if square_to_play is None:
    #    human_move = None
    #    while human_move not in valid_moves:
    #        human_move = human_ai(board=board, square_to_play=square_to_play, player=player)
    #    return human_move
    ...
    for move, square in valid_moves:
        ...
        score = minimax_score(new_board, move, get_opponent(player), 6, not maxing)
